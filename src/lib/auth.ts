...
export const { handlers, signIn, signOut, auth } = NextAuth({
  adapter: {
    ...
    getUserByEmail: (email: string) => db.customer.findUnique({ where: { email } }),
    getUserById: (id: string) => db.customer.findUnique({ where: { id } }),
    getUserByAccount: async (arg: any) => {
      const provider: string | undefined = typeof arg === 'object' && arg ? arg.provider : undefined
      const providerAccountId: string = typeof arg === 'string' ? arg : arg?.providerAccountId
    ...
    updateUser: (data: any) => db.customer.update({ where: { id: data.id }, data }),
    ...
    createVerificationToken: (data: any) => db.verificationToken.create({ data }),
