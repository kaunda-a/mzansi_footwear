...
export function validatePaymentConfig(): void {
  ...
  console.log(`✅ ${enabledProviders.length} payment provider(s) configured:`, 
  ...
  enabledProviders.forEach(config => {
    Object.entries(config.credentials).forEach(([key, value]) => {
      if (!value) {
        missingVars.push(`${config.provider.toUpperCase()}_${key.toUpperCase()}`);
      }
    });
  });
 ...
 */
export function getRecommendedMethods(amount: number, customerLocation?: string): string[] {
