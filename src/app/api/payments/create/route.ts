...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFields = ['amount', 'currency', 'orderId', 'items', 'customer'];
    ...
    const paymentRequest: PaymentRequest = {
      ...
      customer: {
        ...
        firstName: body.customer.firstName || session.user.name?.split(' ')[0] || '',
        lastName: body.customer.lastName || session.user.name?.split(' ').slice(1).join(' ') || '',
      ...
      returnUrl: body.returnUrl || `${process.env.NEXTAUTH_URL}/payment/success`,
      cancelUrl: body.cancelUrl || `${process.env.NEXTAUTH_URL}/payment/cancelled`,
      notifyUrl: body.notifyUrl || `${process.env.NEXTAUTH_URL}/api/payments/webhook`,
    ...
    const response = await paymentManager.createPayment(paymentRequest, preferredProvider);
