...
export async function PUT(request: NextRequest) {
    try {
        ...
        const updatedCustomer = await CustomerService.updateCustomer(session.user.id, body)
        
        if (!updatedCustomer) {
            return NextResponse.json({ error: "Failed to update customer profile" }, { status: 400 })
        }
