...
import { 
  Package, Truck, CheckCircle, XCircle, Clock, Receipt, Calendar, Hash, MapPin, CreditCard, Download
} from 'lucide-react'

function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; className: string; Icon: any }> = {
    PENDING: { label: 'Pending', className: 'bg-yellow-500 text-white', Icon: Clock },
    CONFIRMED: { label: 'Confirmed', className: 'bg-blue-500 text-white', Icon: CheckCircle },
    PROCESSING: { label: 'Processing', className: 'bg-blue-500 text-white', Icon: Package },
    SHIPPED: { label: 'Shipped', className: 'bg-purple-500 text-white', Icon: Truck },
    DELIVERED: { label: 'Delivered', className: 'bg-green-500 text-white', Icon: CheckCircle },
    CANCELLED: { label: 'Cancelled', className: 'bg-red-500 text-white', Icon: XCircle },
    REFUNDED: { label: 'Refunded', className: 'bg-orange-500 text-white', Icon: Receipt },
  }
...
export default async function OrderDetailPage({ params }: any) {
  ...
  if (!order) {
    return (
      <AccountLayout title="Order Not Found" breadcrumbs={[{ label: 'Orders', href: '/account/orders' }, { label: 'Not Found' }]}>
        <Card>
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                  <span className="inline-flex items-center"><Hash className="h-4 w-4 mr-1" />{order.orderNumber}</span>
                  <span className="inline-flex items-center"><Calendar className="h-4 w-4 mr-1" />{new Date(order.createdAt).toLocaleString()}</span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {order.items.map((item: any) => {
                  ...
                  const price = item.totalPrice?.toNumber?.() || item.price || 0
                  return (
              ...
              <div className="flex flex-col items-end gap-1">
                <div className="flex justify-between w-full md:w-1/2 text-sm text-muted-foreground">
                  <span>Subtotal</span>
                  <span>{formatPrice(order.items.reduce((sum: number, it: any) => sum + (it.totalPrice?.toNumber?.() || it.price || 0), 0))}</span>
                </div>
                <div className="flex justify-between w-full md:w-1/2 text-sm text-muted-foreground">
                  <span>Shipping</span>
        ...
        <div className="space-y-6">
            ...
            <CardContent className="text-sm text-muted-foreground">
              {order.shippingAddress ? (
                <div className="space-y-1">
                  <div className="font-medium text-foreground">{order.shippingAddress.firstName} {order.shippingAddress.lastName}</div>
          ...
          <Card>
            ...
            <CardContent className="text-sm">
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <div className="font-medium">{payment ? payment.method : '—'}</div>
              </div>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2"><Truck className="h-4 w-4" /> Tracking</CardTitle>
            </CardHeader>
            <CardContent className="text-sm">
              {order.trackingNumber ? (
                ...
                <div className="text-muted-foreground">No tracking info yet.</div>
              )}
            </CardContent>
          </Card>
        </div>
